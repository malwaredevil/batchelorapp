...
**How to apply:** after any route transplant, grep server code for backtick `/api/` literals and confirm each carries the app prefix; route _definitions_ (relative to mount) are fine, only emitted absolute URLs need the prefix.
