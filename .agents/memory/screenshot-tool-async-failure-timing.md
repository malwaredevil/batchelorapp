...
console logs can include events that happened _after_ the screenshot pixels
were taken.
...
showing — even though the console log in the _same_ tool response clearly
...
of the _settled_ state, you need `runTest` (Playwright, which can
