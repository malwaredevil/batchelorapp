...
- Every webhook endpoint must prevent replay **before** side effects with a delivery/dedup table, transactional idempotency key, or idempotent processing contract. HMAC/token validation may authenticate the caller but never substitutes for replay prevention; a timestamp-based safeguard also needs an enforced bounded window and idempotent handling
