...
// Keep the lock outside the worktree.  The artifact path makes this stable for
// all API builds of this checkout while the digest avoids collisions between
// different workspaces that happen to share the same temp directory.
const buildLockPath = path.join(
  tmpdir(),
  `workspace-api-build-${createHash("sha256").update(artifactDir).digest("hex")}.lock`,
);
  ...
  try {
    for (const entry of await readdir(artifactDir, { withFileTypes: true })) {
      if (
        ...
        entry.name !== path.basename(stagingDir)
      ) {
        await rm(path.resolve(artifactDir, entry.name), {
          recursive: true,
          force: true,
        });
      }
