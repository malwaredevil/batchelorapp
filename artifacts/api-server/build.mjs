...
async function acquireBuildLockGuard(lockPath, deadline) {
...
async function acquireBuildLock(lockPath) {
  ...
  while (true) {
    const releaseGuard = await acquireBuildLockGuard(lockPath, deadline);
    ...
    try {
      try {
        ...
        return async () => {
          const releaseGuard = await acquireBuildLockGuard(
            lockPath,
            Date.now() + BUILD_LOCK_TIMEOUT_MS,
          );
          try {
            await handle.close();
        ...
        try {
          // All cooperating builders hold lockPath.guard while inspecting or
          // mutating this path, so it cannot change between this read and the
          // stale-lock removal below.
    ...
    if (shouldRetry) await delay(BUILD_LOCK_RETRY_MS);
  }
}
...
async function rewriteStagedRuntimePaths(directory, stagingDir, distDir) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await handle.writeFile(rewritten);
    } finally {
      await handle.close();
    }
  }
