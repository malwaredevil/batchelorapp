...
beforeAll(async () => {
  ...
  if (!buildSource.includes(promotionMarker)) {
    throw new Error("API build promotion marker changed; update this test");
  }
  const interruptedBuildSource = buildSource.replace(
    promotionMarker,
...
describe("API build publication safety", () => {
  it("waits for a just-created empty lock instead of deleting it", async () => {
    await resetFixture();
    ...
    await expect(waitForChild(build)).resolves.toMatchObject({ code: 0 });
  ...
  it("serializes concurrent builds while the first owner is stopped", async () => {
    ...
    await waitFor(
      async () => (await readFile(lockPath, "utf8")) === String(firstBuild.pid),
      "first build to own the lock",
    );
    ...
    try {
      ...
      expect(await readFile(lockPath, "utf8")).toBe(String(firstBuild.pid));
    } finally {
