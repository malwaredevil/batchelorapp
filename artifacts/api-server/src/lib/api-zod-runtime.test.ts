    ...
    entries
      .filter(
        (entry) =>
          ...
          entry === ".build-lock-ready" ||
          entry === ".build-lock-release" ||
...
beforeAll(async () => {
  ...
  const lockAcquiredMarker = `  const releaseBuildLock = await acquireBuildLock(
    path.resolve(artifactDir, ".dist-build.lock"),
  );`;
  if (!fixtureBuildSource.includes(lockAcquiredMarker)) {
    throw new Error("API build lock marker changed; update this test");
  }
  const readyAwareBuildSource = fixtureBuildSource.replace(
    lockAcquiredMarker,
    `${lockAcquiredMarker}
  if (process.env.API_BUILD_TEST_READY_FILE) {
    const readyFile = await open(process.env.API_BUILD_TEST_READY_FILE, "w");
     await readyFile.close();
   }
  if (process.env.API_BUILD_TEST_RELEASE_FILE) {
    while (true) {
      try {
        await stat(process.env.API_BUILD_TEST_RELEASE_FILE);
        break;
      } catch (error) {
  ...
  if (!readyAwareBuildSource.includes(promotionMarker)) {
    throw new Error("API build promotion marker changed; update this test");
  }
  const interruptedBuildSource = readyAwareBuildSource.replace(
    promotionMarker,
...
describe("API build publication safety", { timeout: 15_000 }, () => {
  it("releases the lock when its owner is killed", async () => {
    await resetFixture();
    const readyPath = path.join(fixtureDir, ".build-lock-ready");
    const firstBuild = startBuild({ API_BUILD_TEST_READY_FILE: readyPath });
    await waitFor(() => exists(readyPath), "first build to acquire the lock");
  ...
  it("serializes concurrent builds while the first owner is stopped", async () => {
    await resetFixture();
    const readyPath = path.join(fixtureDir, ".build-lock-ready");
    const releasePath = path.join(fixtureDir, ".build-lock-release");
    const firstBuild = startBuild({
      API_BUILD_TEST_READY_FILE: readyPath,
      API_BUILD_TEST_RELEASE_FILE: releasePath,
    });
  ...
  it("cleans stale staging directories without touching published artifacts", async () => {
    await resetFixture();
    const staleStagingDir = path.join(fixtureDir, ".dist-staging-old-pid");
    await mkdir(staleStagingDir);
    await writeFile(path.join(staleStagingDir, "stale.txt"), "stale");

    await expect(waitForChild(startBuild())).resolves.toMatchObject({
      code: 0,
    });
    await expect(exists(staleStagingDir)).resolves.toBe(false);
    await expect(fixtureHasRunnableArtifact()).resolves.toBe(true);
  });
});
