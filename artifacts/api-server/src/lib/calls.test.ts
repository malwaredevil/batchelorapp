...
describe("initiateOutboundCall", () => {
  ...
  it("still places the provider call when seeding context fails", async () => {
  ...
  it("clears context and reports undelivered context when attaching the accepted call fails", async () => {
    mockAttachPendingOutboundCallId.mockRejectedValueOnce(
      new Error("db unavailable"),
    );
