...
function retryPendingOutboundContextCleanup(
  toNumber: string,
  pendingId: string,
): void {
  void (async () => {
...
export async function initiateOutboundCall(
  ...
  try {
    ...
    const cleared = await clearPendingOutboundCallContextBestEffort(
      opts.toNumber,
      pendingId,
    );
    if (!cleared) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 408 || response.status >= 500) {
      throw new OutboundCallIndeterminateError(
        ...
        new Error(`provider returned ambiguous status ${response.status}`),
      );
    }
    logger.error(
      { status: response.status, text },
  ...
  try {
    ...
    if (!cleared) retryPendingOutboundContextCleanup(opts.toNumber, pendingId);
    throw new OutboundCallIndeterminateError(
      { phoneNumber: opts.toNumber, pendingId },
      err,
    );
  ...
  return {
    callId,
    contextAttached,
    pendingOutboundContext: pendingId
      ? { phoneNumber: opts.toNumber, pendingId }
      : undefined,
  };
}
...
async function clearPendingOutcomeContext(
  ...
  try {
    ...
    if (pendingContext) {
      retryPendingOutboundContextCleanup(
        pendingContext.phoneNumber,
        pendingContext.pendingId,
      );
    }
