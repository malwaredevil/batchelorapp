...
async function handleVoice(req: Request, res: Response): Promise<void> {
  ...
  if (!transcript) {
    const normalizedStartupDirection = direction.toLowerCase();
    const isOutboundStartup =
      normalizedStartupDirection === "outbound" ||
      (normalizedStartupDirection === "" && Boolean(callId?.trim()));
  ...
  try {
    if (mayConsumeOutboundContext && callId) {
...
router.post("/webhook", webhookLimiter, async (req: Request, res: Response) => {
  ...
  if (event === "agent.message" && channel === "voice") {
    const startupData = (req.body?.data ?? {}) as Record<string, unknown>;
    const startupTranscript =
      typeof startupData.transcript === "string"
        ? startupData.transcript.trim()
        : "";
    const startupDirection =
      typeof startupData.direction === "string"
        ? startupData.direction.toLowerCase()
        : typeof startupData.callDirection === "string"
          ? startupData.callDirection.toLowerCase()
          : "";
    const startupCallId =
      typeof startupData.callId === "string"
        ? startupData.callId.trim()
        : typeof startupData.call_id === "string"
          ? startupData.call_id.trim()
          : "";
    if (
      !startupTranscript &&
      (startupDirection === "outbound" ||
        (startupDirection === "" && Boolean(startupCallId)))
    ) {
      res.status(200).json({ text: "" });
      return;
    }
