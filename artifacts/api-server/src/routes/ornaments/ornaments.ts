...
async function ornamentListMeta(where: SQL<unknown>, total: number) {
  ...
  return {
    totalPages: total === 0 ? 1 : Math.ceil(total / 1),
    facets: {
...
router.get("/items", async (req, res) => {
    ...
    try {
      ...
      if (rankedIds.length > 0) {
        ...
        const metaWhere = rankedIds.length
          ...
          : baseWhere;
        const meta = await ornamentListMeta(metaWhere, total);
        res.json(
  ...
  const [items, meta] = await Promise.all([
    serializeItems(rows),
    ornamentListMeta(where, Number(total)),
  ]);
  res.json(
    ListOrnamentsResponse.parse({
      items,
