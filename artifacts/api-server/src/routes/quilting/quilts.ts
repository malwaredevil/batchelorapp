...
router.get("/quilts", async (req, res) => {
  ...
  const textSearch = q
    ? createCollectionTextSearch(q, {
        title: [finishedQuilts.name],
        broad: [finishedQuilts.recipient, finishedQuilts.notes],
      })
    : null;
  const baseWhere = textSearch?.where;
  const where = baseWhere
    ? and(baseWhere, isNull(finishedQuilts.deletedAt))
...
router.get("/quilts/:id", async (req, res) => {
  ...
  const [row] = await db
    .select()
    .from(finishedQuilts)
    .where(and(eq(finishedQuilts.id, id), isNull(finishedQuilts.deletedAt)))
    .limit(1);
  if (!row) {
...
router.post("/quilts", upload.single("image"), async (req, res) => {
  ...
  const rawLinkedFabricIds = parseIntegerArray(req.body.linkedFabricIds);
  const rawLinkedPatternIds = parseIntegerArray(req.body.linkedPatternIds);
...
router.post("/quilts/:id/reanalyze", aiLimiter, async (req, res) => {
  ...
  let updated: FinishedQuiltRow;
  try {
    updated = await runQuiltRecognition(id);
  } catch (error) {
    const status = (error as { status?: number }).status;
    if (status) {
      res.status(status).json({
        error:
          error instanceof Error ? error.message : "Unable to analyse quilt.",
      });
      return;
    }
    throw error;
...
export async function bulkReanalyzeQuilts(
  ...
  return runQuiltingBulkReanalysis(ids, runQuiltRecognition);
}
...
export async function deleteQuiltById(id: number): Promise<boolean> {
  ...
  const now = new Date();
  // Cascade soft-delete supplemental images, then the quilt row itself.
...
router.post("/quilts/:id/images", upload.single("image"), async (req, res) => {
  ...
  const cleanBuffer = await stripMetadata(file.buffer, sniffedType);
  const storagePath = await uploadImage(cleanBuffer, sniffedType);
  ...
  scheduleQuiltingRecognition("quilt", id);
  res.status(201).json({
    id: image.id,
...
router.patch("/quilts/:id/images/:imageId", async (req, res) => {
  ...
  if (body.position !== undefined) scheduleQuiltingRecognition("quilt", id);
  res.json({
    id: image.id,
    url: `/api/quilting/quilts/${id}/images/${image.id}`,
    label: image.label,
