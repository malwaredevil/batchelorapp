    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="bg-slate-800 w-12 flex flex-col items-center py-3 gap-2 shrink-0">
          {tools.map(({ id, Icon }, i) => (
            <button
              key={id}
              className={`w-9 h-9 rounded-xl flex items-center justify-center ${i === 0 ? "bg-teal-500" : "bg-white/10"}`}
            >
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex items-center justify-center bg-slate-200 p-2">
            <div
              className="border border-slate-400 shadow-lg bg-white"
              style={{
                display: "grid",
              ...
              {Array.from({ length: GRID }).map((_, r) =>
                Array.from({ length: GRID }).map((_, c) => (
                  <div
                    key={`${r},${c}`}
                    style={{
                      backgroundColor: cellColors[`${r},${c}`] ?? "#FDFEFE",
      ...
      <div className="bg-slate-900 px-3 pt-2 pb-3 shrink-0">
        <div className="flex items-center justify-between mb-2">
          ...
          <button className="flex items-center gap-0.5 text-teal-400 text-[10px]">
            Add fabric <ChevronRight size={10} />
          </button>
        </div>
