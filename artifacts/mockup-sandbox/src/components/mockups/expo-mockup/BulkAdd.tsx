import { ArrowLeft, Zap, CheckCircle2, Loader2, AlertCircle } from "lucide-react";
...
function StatusBar() {
  return (
    <div style={{ height: STATUS_BAR_H }} className="flex items-center justify-between px-5 shrink-0 absolute top-0 left-0 right-0 z-20">
      <span className="text-white text-[11px] font-semibold tracking-wide drop-shadow">12:47</span>
      <div className="flex items-center gap-1.5">
        <div className="flex gap-[2px] items-end h-3">
          {[3, 4, 5, 6].map((h, i) => (
            <div key={i} style={{ height: `${h * 2}px` }} className="w-[3px] bg-white rounded-sm opacity-90" />
          ))}
        </div>
...
export function BulkAdd() {
  return (
    <div className="flex flex-col w-full h-screen overflow-hidden relative" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div className="absolute inset-0 bg-slate-900">
        ...
        <div className="absolute inset-0 opacity-60"
          style={{
            background: "linear-gradient(160deg, #1a2a1a 0%, #2d3b2d 30%, #1a1a2e 70%, #16213e 100%)"
          }}
        />
        ...
        <div className="absolute left-10 right-10 top-[44%] h-px bg-teal-400/50"
      ...
      <div className="relative z-10 flex items-center justify-between px-4 pt-10 pb-3">
        ...
        <div className="flex items-center gap-1.5 bg-black/40 backdrop-blur-sm rounded-full px-3 py-1.5">
          ...
          <span className="text-[12px] font-semibold text-white">AI Auto-Fill ON</span>
        </div>
          ...
          {queue.map((item) => (
            <div key={item.id} className="flex items-center gap-3 bg-white/5 rounded-2xl px-3 py-2.5">
              ...
              <div className="flex-1 min-w-0">
                {item.status === "done" && (
                  <>
                    <p className="text-white text-[13px] font-medium truncate">{item.name}</p>
                    <p className="text-slate-400 text-[11px]">Saved to collection</p>
                  </>
                )}
                {item.status === "processing" && (
                  <>
                    <p className="text-slate-300 text-[13px] font-medium">Analysing with AI…</p>
                    <p className="text-teal-400 text-[11px]">Filling in all details</p>
                  </>
                )}
                {item.status === "queued" && (
                  <>
                    <p className="text-slate-400 text-[13px]">Queued</p>
                    <p className="text-slate-500 text-[11px]">Waiting to upload</p>
                  </>
                )}
              </div>
              {item.status === "done" && <CheckCircle2 size={20} className="text-emerald-400 shrink-0" />}
              {item.status === "processing" && <Loader2 size={20} className="text-teal-400 animate-spin shrink-0" />}
              {item.status === "queued" && <div className="w-5 h-5 rounded-full border-2 border-slate-600 shrink-0" />}
            </div>
          ))}
        </div>
