...
function BottomTabBar({ active = "fabrics" }: { active?: string }) {
  ...
  return (
    <div
      className="flex items-center bg-white border-t border-slate-200 shrink-0"
      ...
      {tabs.map((t) => {
        ...
        return (
          ...
          >
            ...
            {isActive && (
              <div
                className="absolute bottom-[56px] h-0.5 w-10 bg-teal-600 rounded-full"
...
export function FabricsList() {
  return (
    <div
      className="flex flex-col w-full h-screen bg-slate-50 overflow-hidden"
    ...
    >
      ...
      <div className="bg-white px-4 pt-3 pb-2 shadow-sm shrink-0">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
          {["All (46)", "Cotton", "Flannel", "Linen", "Voile"].map(
            (chip, i) => (
              <button
                key={chip}
