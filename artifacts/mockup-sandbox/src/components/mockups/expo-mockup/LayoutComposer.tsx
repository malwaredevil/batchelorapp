...
function MiniBlock({
  ...
  return (
    <div
      className={`w-full aspect-square border rounded-sm overflow-hidden ${selected ? "border-teal-400 shadow-sm" : "border-slate-300"}`}
...
const blocks = [
  {
    id: "ohio-star",
    name: "Ohio Star",
    pattern: ["#2D4A6B", "#FDFEFE", "#FDFEFE", "#2D4A6B"],
  },
  {
    id: "log-cabin",
    name: "Log Cabin",
    pattern: ["#B5526B", "#F0A0B4", "#F0A0B4", "#B5526B"],
  },
  {
    id: "pinwheel",
    name: "Pinwheel",
    pattern: ["#2E7D4F", "#82C99A", "#82C99A", "#2E7D4F"],
  },
];
...
export function LayoutComposer() {
  return (
    <div
      className="flex flex-col w-full h-screen bg-slate-100 overflow-hidden"
    ...
    >
      ...
      <div className="bg-slate-800 flex items-center gap-2 px-3 py-1.5 shrink-0">
        {[
          ...
          <button
            key={label}
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-medium ${i === 0 ? "bg-teal-600 text-white" : "bg-white/10 text-slate-300"}`}
          >
            <Icon size={12} />
            {label}
          </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-24 bg-slate-900 flex flex-col shrink-0 overflow-hidden">
          <div className="px-2 pt-2 pb-1">
            <p className="text-slate-400 text-[9px] font-semibold uppercase tracking-widest">
              Blocks
            </p>
          </div>
          <div className="flex-1 overflow-y-auto px-2 space-y-2 pb-2">
            {blocks.map((b) => (
              <button key={b.id} className="w-full">
                <div
                  className={`w-full aspect-square border-2 rounded-lg overflow-hidden ${b.id === "ohio-star" ? "border-teal-400" : "border-transparent"}`}
                  style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}
                >
                  {b.pattern.map((c, i) => (
                    <div key={i} style={{ backgroundColor: c }} />
                ...
                <p className="text-[8px] text-slate-400 mt-0.5 text-center leading-tight">
                  {b.name}
                </p>
              </button>
            ))}
      ...
      <div className="bg-slate-900 px-3 pt-1.5 pb-2.5 shrink-0">
        <p className="text-slate-400 text-[9px] font-semibold uppercase tracking-widest mb-1.5">
          Fabrics in layout
        </p>
        <div className="flex gap-1.5">
          {[
