...
function StatusBar() {
  return (
    <div
      ...
      style={{ height: 28 }}
    >
      <span className="text-amber-100 text-[11px] font-semibold tracking-wide drop-shadow">
        12:52
      </span>
    ...
    >
      ...
      <div className="relative shrink-0" style={{ height: 290 }}>
        <div
          ...
          style={{
            background:
              "linear-gradient(160deg, #2D5A2D 0%, #8FBC8F 50%, #C5DEC5 100%)",
          }}
        >
        ...
        <div
          ...
          style={{
            background: "linear-gradient(to top, #1C1007, transparent)",
          }}
        />
        ...
        <div className="absolute bottom-3 left-4 flex gap-1.5">
          {["🏺", "🏺", "🏺"].map((e, i) => (
            <div
              key={i}
              className={`w-10 h-10 rounded-lg flex items-center justify-center border-2 text-lg ${i === 0 ? "border-amber-400" : "border-transparent"}`}
              style={{ backgroundColor: "rgba(0,0,0,0.4)" }}
            >
              {e}
            </div>
          ...
          <button
            className="w-10 h-10 rounded-lg flex items-center justify-center border-2 border-dashed border-amber-700"
            style={{ backgroundColor: "rgba(0,0,0,0.4)" }}
          >
        ...
        <div className="px-4 pt-3 pb-24">
            ...
            <div className="flex gap-2">
              {colors.map((c, i) => (
                <div key={i} className="flex flex-col items-center gap-1">
                  <div
                    className="w-9 h-9 rounded-xl border border-amber-900"
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {aiTags.map((t) => (
              <span
                key={t}
                className="px-2.5 py-1 rounded-full text-[11px] font-medium border border-amber-800 text-amber-400"
          ...
          {/* AI description */}
          <div
            className="mt-4 rounded-2xl p-4 border border-amber-900"
          ...
          >
            ...
            <p className="text-[13px] leading-relaxed text-amber-200">
              A refined celadon-glazed stoneware bowl with a subtle crackling
              surface and pale jade-green glaze. The form shows Hamada's
              characteristic restraint — slightly irregular rim, foot ring with
              exposed clay body.
            </p>
          </div>
          ...
          {/* Details */}
          <div
            className="mt-3 rounded-2xl border border-amber-900 overflow-hidden"
          ...
          >
            {[
              ...
              <div
                key={row.label}
                className={`flex items-center justify-between px-4 py-3 ${i < arr.length - 1 ? "border-b border-amber-950" : ""}`}
              >
      ...
      {/* Bottom action bar */}
      <div
        className="absolute bottom-0 left-0 right-0 px-4 py-3 flex items-center gap-3 border-t border-amber-950 shadow-lg"
