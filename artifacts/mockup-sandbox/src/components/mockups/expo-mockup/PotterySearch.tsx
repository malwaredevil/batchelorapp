...
function VerdictBadge({ verdict }: { verdict: Verdict }) {
  ...
  const labels: Record<Verdict, string> = {
    yes: "Likely duplicate",
    maybe: "Possible match",
    no: "New to collection",
  };
  return (
    <span
      className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${styles[verdict]}`}
    >
      {labels[verdict]}
    </span>
  );
...
const matches = [
  ...
  {
    id: 2,
    name: "Green Glaze Bowl",
    maker: "Unknown",
    similarity: 71,
    verdict: "maybe" as Verdict,
    emoji: "🏺",
    color: "#5A8F5A",
  },
  {
...
export function PotterySearch() {
  return (
    ...
    >
      ...
      {/* Header */}
      <div
        className="flex items-center gap-3 px-4 pt-3 pb-3 shrink-0"
        style={{
      ...
      >
        ...
        <div>
          <h1 className="text-[17px] font-bold text-amber-100">
            AI Photo Compare
          </h1>
          <p className="text-[11px] text-amber-600">
            Find duplicates in your collection
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        ...
        >
          <div
            ...
            style={{ background: "linear-gradient(135deg, #2D5A2D, #8FBC8F)" }}
          >
            ...
            <button
              className="absolute bottom-2 right-2 flex items-center gap-1 rounded-lg px-2 py-1"
          ...
          {[
            {
              label: "Same pattern?",
              verdict: "yes" as Verdict,
              copy: "You very likely already own this pattern.",
            },
            {
              label: "Exact piece?",
              verdict: "maybe" as Verdict,
              copy: "This could be a piece you already have — worth a closer look.",
            },
        ...
        <div className="space-y-2">
          {matches.map((m) => (
            ...
            >
              <div
                className="w-14 h-14 rounded-xl flex items-center justify-center shrink-0 text-3xl border border-amber-900"
                style={{
