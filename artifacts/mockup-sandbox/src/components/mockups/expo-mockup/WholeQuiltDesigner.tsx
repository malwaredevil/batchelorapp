...
export function WholeQuiltDesigner() {
  return (
    <div
      className="flex flex-col w-full h-screen bg-slate-900 overflow-hidden"
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-slate-800 shrink-0">
        {[
          ...
          <button
            key={i}
            className={`w-9 h-9 rounded-xl flex items-center justify-center ${active ? "bg-teal-500" : "bg-white/10"}`}
          >
      ...
      <div className="bg-slate-800 px-3 py-2 shrink-0 border-t border-slate-700">
        ...
        <div className="space-y-1">
          {["Backing fabric", "Batting", "Quilt top", "Border"].map(
            (name, i) => (
              <div
                key={name}
                className={`flex items-center gap-2 rounded-lg px-2 py-1.5 ${i === 2 ? "bg-teal-900/40" : "bg-white/5"}`}
              >
                <GripVertical size={10} className="text-slate-500 shrink-0" />
                <div
                  className="w-4 h-4 rounded shrink-0"
                  style={{ backgroundColor: fabricColors[i * 3] }}
                />
                <span
                  className={`text-[11px] flex-1 ${i === 2 ? "text-teal-300 font-medium" : "text-slate-400"}`}
                >
                  {name}
                </span>
