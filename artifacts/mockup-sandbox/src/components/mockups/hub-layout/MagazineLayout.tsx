...
export default function MagazineLayout() {
  ...
  const spotlightItems = [
    ...
    {
      name: "Speckled Raku Vase",
      app: "Pottery",
      tag: "Raku fired",
      color:
        "from-stone-100 to-stone-200 dark:from-stone-800/30 dark:to-stone-700/20",
    },
  ];
        ...
        <header className="sticky top-0 z-10 bg-background/90 backdrop-blur-md border-b border-border px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button className="hidden md:flex items-center gap-2 text-sm text-muted-foreground border border-border rounded-lg px-3 py-1.5 hover:bg-muted transition-colors">
              ...
              <kbd className="ml-2 text-[10px] border border-border rounded px-1 py-0.5 bg-muted font-mono">
                ⌘K
              </kbd>
            </button>
            <button
        ...
        <main className="max-w-6xl mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-2 space-y-4">
              {/* Spotlight card */}
              <div
                className={`relative rounded-2xl bg-gradient-to-br ${current.color} border border-border overflow-hidden`}
              >
                <div className="p-6 pb-0 flex items-start justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-xs bg-background/60 px-2 py-1 rounded-md font-medium">
                        {current.app}
                      </span>
                  ...
                  <div className="flex gap-1.5">
                    <button
                      onClick={() => setSpotlight((s) => s + 1)}
                      className="p-2 rounded-lg bg-background/40 hover:bg-background/60 text-muted-foreground transition-colors"
                    >
              ...
              <div className="grid grid-cols-2 gap-4">
                {/* Pottery stats */}
                <div
                  className="bg-card border border-border rounded-xl p-4 group hover:shadow-sm transition-shadow cursor-pointer"
                  onClick={() =>
                    setExpanded(expanded === "pottery" ? null : "pottery")
                  }
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <ArrowRight
                      className={`w-3.5 h-3.5 text-muted-foreground transition-transform ${expanded === "pottery" ? "rotate-90" : ""}`}
                    />
                  </div>
                  <div className="flex items-end gap-1">
                    <span className="text-4xl font-bold">163</span>
                    <span className="text-muted-foreground text-sm mb-1 ml-1">
                      pieces
                    </span>
                  </div>
                  ...
                  {expanded === "pottery" && (
                    <div className="mt-3 pt-3 border-t border-border space-y-1.5">
                      {["Blue celadon bowl", "Raku vase", "Speckled mug"].map(
                        (i, idx) => (
                          <div
                            key={idx}
                            className="flex items-center gap-2 text-sm py-1 hover:bg-muted/50 rounded px-1 cursor-pointer"
                          >
                ...
                <div
                  className="bg-card border border-border rounded-xl p-4 group hover:shadow-sm transition-shadow cursor-pointer"
                  onClick={() =>
                    setExpanded(expanded === "quilting" ? null : "quilting")
                  }
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                    ...
                    <ArrowRight
                      className={`w-3.5 h-3.5 text-muted-foreground transition-transform ${expanded === "quilting" ? "rotate-90" : ""}`}
                    />
                  </div>
                  <div className="flex items-end gap-1">
            ...
            <div className="col-span-1 space-y-4">
              ...
              <div className="bg-card border border-border rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs bg-rose-100 dark:bg-rose-900/40 text-rose-600 dark:text-rose-400 px-1.5 py-0.5 rounded-full font-medium">
                    3
                  </span>
                </div>
                {[
                  ...
                  <div key={i} className="flex items-center gap-2.5 py-1.5">
                    <div
                      className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${item.status === "ordered" ? "bg-emerald-400" : "bg-rose-400"}`}
                    />
              ...
              <div className="bg-card border border-border rounded-xl p-4">
                ...
                {[
                  ...
                  <div
                    key={i}
                    className="flex items-start gap-2 py-1.5 cursor-pointer hover:bg-muted/40 rounded px-1 -mx-1 transition-colors"
                  >
                    <div
                      className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${item.app === "pottery" ? "bg-amber-400" : "bg-violet-400"}`}
                    />
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    },
                    {
                      ...
                      icon: Shirt,
                      color:
                        "hover:bg-violet-50 dark:hover:bg-violet-900/20 hover:border-violet-200 dark:hover:border-violet-800",
                    },
                    {
                      label: "Block",
                      icon: Scissors,
                    ...
                    <button
                      key={a.label}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-lg border border-border text-xs font-medium transition-colors ${a.color}`}
                    >
