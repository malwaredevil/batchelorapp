...
function Toggle({
  ...
  return (
    <button
      ...
      className={`relative w-10 h-6 rounded-full transition-colors flex-shrink-0 ${value ? "bg-primary" : "bg-muted-foreground/30"}`}
    >
      <span
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        <section className="space-y-3">
          ...
          <Input
            ...
            placeholder="e.g. 94102 or Paris, FR"
            hint="Used to fetch current conditions from Open-Meteo (no API key needed)."
          />
            ...
            <div className="flex gap-2">
              {["C", "F"].map((u) => (
                <button
                  key={u}
                  onClick={() => setUnits(u)}
                  className={`flex-1 py-2 rounded-lg border text-sm font-medium transition-colors ${units === u ? "bg-primary text-primary-foreground border-transparent" : "border-border text-muted-foreground hover:bg-muted"}`}
                >
                  °{u}
                </button>
              ))}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        <section className="space-y-3">
          ...
          <Input
            ...
            hint="Paste any RSS or Atom feed URL."
          />
          <div className="space-y-1">
            ...
            {[
              ...
              {
                l: "Pottery Making Illustrated",
                u: "https://ceramicstoday.com/feed/",
              },
            ].map((f) => (
              <button
                key={f.l}
                onClick={() => setFeedUrl(f.u)}
...
function WebsitePanel({
  ...
  const [url, setUrl] = useState(
    type === "etsy"
      ? "https://www.etsy.com/shop/MyCeramicsShop"
      : "https://example.com",
  );
  const [title, setTitle] = useState(
    type === "etsy" ? "My Etsy Shop" : "My Favourite Site",
  );
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
        ...
        <div>
          <div className="text-sm font-semibold">
            {type === "etsy" ? "Etsy Shop" : "Website Bookmark"}
          </div>
          <div className="text-xs text-muted-foreground">Widget settings</div>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        <section className="space-y-3">
          ...
          <Input
            ...
            hint={
              type === "etsy"
                ? "Paste your Etsy shop, search, or favourite listing URL."
                : "Any public website URL. Note: some sites block embedding."
            }
          />
        </section>
        <section className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                Embed the page in the widget (if the site allows it)
              </div>
            </div>
...
const SAMPLE_WIDGETS = [
  {
    ...
    icon: Wind,
    sub: "17°C · Partly cloudy · Reichenbach",
    accent: "bg-sky-100 dark:bg-sky-900/40 text-sky-600 dark:text-sky-400",
    panel: "weather" as PanelWidget,
  },
  {
    id: "rss",
    label: "RSS Feed",
    icon: Rss,
    sub: "Ravelry Blog · 5 items",
    accent:
      "bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400",
    panel: "rss" as PanelWidget,
  },
  {
    id: "etsy",
    label: "Etsy Shop",
    icon: ShoppingBag,
    sub: "MyCeramicsShop · live preview",
    accent: "bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-400",
    panel: "etsy" as PanelWidget,
  },
  {
    id: "website",
    label: "Website Bookmark",
    icon: Globe,
    sub: "example.com · link card",
    accent:
      "bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400",
    panel: "website" as PanelWidget,
  },
];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans p-6 flex items-start justify-center gap-6">
      ...
      <div className="w-80 flex-shrink-0 space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Click the ⚙ icon on any widget to configure it. Settings sync to
            your account and persist across devices.
          </p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden divide-y divide-border">
          {SAMPLE_WIDGETS.map((w) => (
            <div
              key={w.id}
              className={`flex items-center gap-3 px-4 py-3.5 hover:bg-muted/30 transition-colors ${activePanel === w.panel ? "bg-primary/5" : ""}`}
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${w.accent}`}
              >
              ...
              <button
                onClick={() =>
                  setActivePanel(activePanel === w.panel ? null : w.panel)
                }
        ...
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-3.5 flex gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-emerald-600/80 dark:text-emerald-500 mt-0.5">
              Widget positions, preferences, and custom URLs persist across
              sessions and devices. You never have to reset them.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="w-80 flex-shrink-0 bg-card border border-border rounded-xl overflow-hidden h-[600px]">
          ...
          <div className="h-full flex flex-col items-center justify-center gap-3 p-8 text-center">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Click the ⚙ icon next to any widget to see its settings here.
              </div>
            </div>
          </div>
        )}
        ...
        {activePanel === "rss" && (
          <RssPanel onClose={() => setActivePanel(null)} />
        )}
        {activePanel === "etsy" && (
          <WebsitePanel onClose={() => setActivePanel(null)} type="etsy" />
        )}
        {activePanel === "website" && (
          <WebsitePanel onClose={() => setActivePanel(null)} type="website" />
        )}
      </div>
    </div>
  );
