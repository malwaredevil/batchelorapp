...
import {
  ...
  useCreateMagnetCategory,
  useUpdateMagnetCategoryColors,
  useRenameMagnetCategory,
  useDeleteMagnetCategory,
  useDeleteMagnetUnusedCategories,
  useMergeMagnetCategory,
  getListMagnetCategoriesQueryKey,
  getListMagnetsQueryKey,
  type MagnetsCategory as Category,
...
function CategoryColorEditor({ category }: { category: Category }) {
  ...
  const updateColors = useUpdateMagnetCategoryColors();
  ...
  const handleSave = async () => {
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: getListMagnetCategoriesQueryKey(),
      });
...
function CategoryActionMenu({
  ...
  const queryClient = useQueryClient();
  const deleteCat = useDeleteMagnetCategory();
  const renameCat = useRenameMagnetCategory();
  const mergeCat = useMergeMagnetCategory();
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        `Are you sure you want to delete "${category.name}"? This will remove it from all magnets.`,
      )
    )
      return;
    try {
      ...
      <Dialog open={mergeOpen} onOpenChange={setMergeOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Move all magnets from "{category.name}" into another category,
              then delete "{category.name}".
            </DialogDescription>
          </DialogHeader>
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full border flex items-center justify-center ${
                      mergeTargetId === c.id ? "border-primary" : "border-input"
                    }`}
                  >
                    {mergeTargetId === c.id && (
...
export default function MagnetCategoriesPage() {
  const { data: categories, isLoading } = useListMagnetCategories();
  const queryClient = useQueryClient();
  const createCategory = useCreateMagnetCategory();
  const deleteUnused = useDeleteMagnetUnusedCategories();
  ...
  const handleCleanup = async () => {
    if (!confirm("Delete all categories that aren't assigned to any magnets?"))
      return;
    try {
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Organize your magnets with color-coded tags
          </p>
        </div>
          ...
          >
            <div className="flex-1 min-w-[200px]">
              ...
              <Input
                {...form.register("name")}
                autoFocus
                placeholder="e.g. Vintage, Travel, Local…"
              />
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 rounded-xl border border-dashed border-border bg-card">
          ...
          <p className="text-muted-foreground mt-1 max-w-sm mx-auto">
            Create categories to help filter and organize your magnets.
          </p>
          <Button
            className="mt-4"
            variant="outline"
              ...
              <div className="flex items-center gap-4">
                ...
                {cat.count !== undefined && (
                  <span className="text-xs text-muted-foreground">
                    {cat.count} magnet{cat.count !== 1 ? "s" : ""}
                  </span>
                )}
              </div>
