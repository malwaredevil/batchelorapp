...
vi.mock("@workspace/api-client-react", () => ({
  ...
  getFabric: vi.fn(),
  appendScreenshotToken: (url: string) => url,
}));
...
afterEach(() => {
  ...
  vi.mocked(fetchBinaryResource).mockReset();
});

describe("resolveLayoutFabricExportData", () => {
  it("fetches a referenced fabric missing from the first page", async () => {
    const { getFabric } = await import("@workspace/api-client-react");
    vi.mocked(getFabric).mockResolvedValue({
      id: 201,
      name: "Far page fabric",
      imageUrl: "/fabrics/201.jpg",
      tileImageUrl: "/fabrics/201-tile.png",
    } as never);
    const result = await resolveLayoutFabricExportData(
      { rows: 1, cols: 1, cells: [{ blockId: 1, rotation: 0 }] },
      new Map([[1, { id: 1, gridSize: 1, cells: ["fab:201"] }]]),
      {},
    );
    expect(getFabric).toHaveBeenCalledWith(201);
    expect(result.fabricUrlMap[201]).toBe("/fabrics/201.jpg");
    expect(result.fabricNames[201]).toBe("Far page fabric");
    vi.mocked(fetchBinaryResource).mockResolvedValue(
      new Blob(["fabric"], { type: "image/png" }),
    );
    const svg = await buildEmbeddedLayoutSvgString(
      { rows: 1, cols: 1, cells: [{ blockId: 1, rotation: 0 }] },
      new Map([[1, { id: 1, gridSize: 1, cells: ["fab:201"] }]]),
      100,
      result.fabricUrlMap,
  ...
  it("fails explicitly when a referenced fabric cannot be resolved", async () => {
    const { getFabric } = await import("@workspace/api-client-react");
    vi.mocked(getFabric).mockRejectedValue(new Error("not found"));
    await expect(
      resolveLayoutFabricExportData(
        { rows: 1, cols: 1, cells: [{ blockId: 1, rotation: 0 }] },
        new Map([[1, { id: 1, gridSize: 1, cells: ["fab:999"] }]]),
        {},
      ),
    ).rejects.toBeInstanceOf(FabricResolutionError);
  });
});
...
describe("buildLayoutSvgString", () => {
  it("uses preview pattern geometry for full cells, block subcells, and trim", () => {
    const layout = {
      rows: 2,
      cols: 2,
      cells: [
        { blockId: 7, rotation: 90 as const },
        { blockId: 7, rotation: 0 as const },
        { blockId: null, rotation: 0 as const },
        { blockId: 7, rotation: 270 as const },
      ],
    ...
    const blockMap = new Map([
      [7, { id: 7, gridSize: 2, cells: ["fab:11", "fab:11", "#abc", "#def"] }],
    ]);
    const svg = buildLayoutSvgString(layout, blockMap, 800, {
      11: "/fabrics/11.jpg",
      22: "/fabrics/22.jpg",
      33: "/fabrics/33.jpg",
    });
    const { cellPx, width, height } = getLayoutSvgGeometry(layout, 800);
    const patternPx = cellPx / DEFAULT_FABRIC_TILE_REPEATS;

    expect(svg).toContain(`<svg width="${width}" height="${height}"`);
    for (const id of [11, 22, 33]) {
      expect(svg).toContain(
        `<pattern id="fab-${id}" patternUnits="userSpaceOnUse" width="${patternPx}" height="${patternPx}">`,
      );
      expect(svg).toContain(
        `<image href="/fabrics/${id}.jpg" width="${patternPx}" height="${patternPx}"`,
      );
    }
...
describe("buildEmbeddedLayoutSvgString", () => {
  ...
  it("embeds referenced fabric images as self-contained data URLs", async () => {
    ...
    expect(fetchBinaryResource).toHaveBeenCalledWith(
      "/fabrics/11.jpg",
  ...
  it("identifies a fabric image that cannot be embedded", async () => {
    vi.mocked(fetchBinaryResource).mockRejectedValue(new Error("HTTP 403"));
  ...
  it("identifies a fabric when its image request is rejected", async () => {
    vi.mocked(fetchBinaryResource).mockRejectedValue(new TypeError("Failed"));
  ...
  it("aborts a fabric response whose body stalls after headers", async () => {
    vi.useFakeTimers();
    vi.mocked(fetchBinaryResource).mockImplementation(
