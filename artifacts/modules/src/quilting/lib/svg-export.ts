...
export type FabricExportResolution = {
...
export class FabricResolutionError extends Error {
  ...
  constructor(fabricId: number) {
    super(
      `Couldn’t resolve fabric #${fabricId} for export. The fabric may have been deleted or its photo is unavailable.`,
    );
    this.name = "FabricResolutionError";
    this.fabricId = fabricId;
  }
}

/** Resolve every fabric used by a layout, fetching only IDs absent from the list cache. */
export async function resolveLayoutFabricExportData(
  layout: LayoutExportInput,
  blockMap: Map<number, LayoutExportBlock>,
  existingUrlMap: FabricUrlMap,
  existingNames: Record<number, string> = {},
): Promise<FabricExportResolution> {
    ...
    ids
      ...
      .map(async (id) => {
        ...
        try {
          ...
          throw new FabricResolutionError(id);
        }
        const url = fabric.imageUrl || fabric.tileImageUrl;
        if (!url) throw new FabricResolutionError(id);
        urlMap[id] = appendScreenshotToken(url);
        names[id] = fabric.name;
      }),
  );
  // A map supplied by the caller may contain a stale/missing entry. Never
  // allow the export builder to turn that reference into a grey placeholder.
  for (const id of ids) {
    if (!urlMap[id]) throw new FabricResolutionError(id);
  }
  return { fabricUrlMap: urlMap, fabricNames: names };
}
...
export function buildBlockSvgString(
  ...
  const fabricPatternPx = getFabricPatternSize(cellPx * gridSize);
  const shapes: string[] = [];
  ...
  if (fabricUrlMap) {
    ...
    if (ids.length > 0) {
      const patterns = ids
        .map(
          (id) =>
            `<pattern id="fab-${id}" patternUnits="userSpaceOnUse" width="${fabricPatternPx}" height="${fabricPatternPx}">` +
            `<image href="${fabricUrlMap[id]}" width="${fabricPatternPx}" height="${fabricPatternPx}" preserveAspectRatio="xMidYMid slice"/>` +
            `</pattern>`,
        )
        .join("");
...
export function buildLayoutSvgString(
  ...
  const { cellPx, sashPx, borderPx, width, height } = geometry;
  const fabricPatternPx = getFabricPatternSize(cellPx);
  ...
  const patterns = fabricIds
    .map(
      (id) =>
        `<pattern id="fab-${id}" patternUnits="userSpaceOnUse" width="${fabricPatternPx}" height="${fabricPatternPx}">` +
        `<image href="${fabricUrlMap[Number(id)]}" width="${fabricPatternPx}" height="${fabricPatternPx}" preserveAspectRatio="xMidYMid slice"/>` +
        `</pattern>`,
    )
    .join("");
  const parts = [
...
async function fetchFabricImage(url: string): Promise<Blob> {
  ...
  try {
    const blob = await fetchBinaryResource(url, {
...
export async function buildEmbeddedLayoutSvgString(
  ...
  const missingId = ids.find((id) => !fabricUrlMap[Number(id)]);
  if (missingId) throw new FabricResolutionError(Number(missingId));
...
export async function downloadCollectionImage(
  ...
  try {
    const blob = await fetchBinaryResource(url, { credentials: "same-origin" });
