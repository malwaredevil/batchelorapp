...
type FabricUrlMap = Record<string, string>;
...
async function fetchFabricImage(url: string): Promise<Response> {
...
export async function buildEmbeddedLayoutSvgString(
  ...
  const embeddedEntries = await Promise.all(
    ids.map(async (id) => {
      try {
        const response = await fetchFabricImage(fabricUrlMap[Number(id)]);
