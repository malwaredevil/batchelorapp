...
import { CellShape } from "@/quilting/components/CellShape";
import {
  FabricPicker,
  computeFabricTally,
  buildFabricUrlMap,
...
// ParsedCell and parseCell are imported from @/quilting/lib/cell-parser above.
...
// encodeXline and applyDiagClip are imported from @/quilting/lib/cell-parser above.
...
// CellShape is imported from @/quilting/components/CellShape above.
...
export default function BlockDesigner() {
  ...
  usePageAssistantContext(
    ...
    `Block Designer page (${templateMode ? "block library template" : "block"} ${
      isNew ? "creation" : `editing ${formatElaineContextEntity({ entity: "block", id: blockId!, label: existing?.name ?? "" })}`
  ...
  const [popWheeling, setPopWheeling] = useState(false);
  const popWheelTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    ...
    mutation: {
      onSuccess: (data) => {
        ...
        exitAfterSaveRef.current
          ? ((exitAfterSaveRef.current = false), navigate("/quilting/blocks"))
          : (toast.success("Block design saved!"),
            navigate(`/quilting/blocks/${data.id}`));
      },
      onError: () => {
        ...
        createTemplate.mutate(
          ...
          },
          {
            onSuccess: (data) => {
              ...
              if (exitAfterSaveRef.current) {
                exitAfterSaveRef.current = false;
                navigate("/quilting/library/blocks");
              } else {
                toast.success("Template saved!");
                navigate(`/quilting/library/blocks/${data.id}/edit`);
              }
            },
            onError: () => {
              exitAfterSaveRef.current = false;
          ...
          >
            ...
            <div
              ...
              onWheel={(e) => {
                ...
                setPopWheeling(true);
                if (popWheelTimerRef.current)
                  clearTimeout(popWheelTimerRef.current);
                popWheelTimerRef.current = setTimeout(
                  () => setPopWheeling(false),
                  150,
                );
              }}
              onPointerDown={(e) => {
              ...
              >
                <div
                  style={{
                    ...
                    transition:
                      popDragRef.current || popWheeling
                        ? "none"
                        : "transform 0.05s",
                  }}
                >
