...
export default function LayoutComposer() {
  ...
  usePageAssistantContext(
    ...
    `Layout Composer page (layout ${
      isNew ? "creation" : `editing ${formatElaineContextEntity({ entity: "layout", id: layoutId!, label: existing?.name ?? "" })}`
  ...
  const createLayout = useCreateLayout({
    mutation: {
      onSuccess: (data) => {
        ...
        setIsDirty(false);
        navigate(`/quilting/layouts/${data.id}`);
      },
    ...
    mutation: {
      onSuccess: () => {
        ...
        if (exitAfterSaveRef.current) {
          ...
          navigate(
            layoutId ? `/quilting/layouts/${layoutId}` : "/quilting/layouts",
          );
        }
      },
