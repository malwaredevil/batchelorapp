...
function AerialViewPanel({ destination }: { destination: string }) {
  const { data, isLoading, error } = useQuery({
    queryKey: ["destination-aerial-view", destination],
    queryFn: () => getAerialViewInfo(destination),
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
        <Spinner className="h-4 w-4" /> Looking for an aerial view…
      </div>
    );
  }
  if (error) {
    return (
      <p className="py-3 text-sm text-muted-foreground">
        Couldn't check for an aerial view right now.
      </p>
    );
  }
  if (data?.state === "ACTIVE" && data.videoUrl) {
    return (
      <video
        ...
        controls
        className="w-full max-h-72 rounded-md border border-border/40 my-2"
      />
    );
  }
  return (
    <p className="py-3 text-sm text-muted-foreground">
      {data?.state === "PROCESSING"
        ? "This aerial view is rendering for the first time — check back again shortly."
        : "No aerial flyover is available for this destination yet."}
    </p>
  );
}
...
function TripTimelineRow({ trip }: { trip: Trip }) {
  ...
  return (
    <Link href={`/travels/trips/${trip.id}`}>
...
function DestinationCard({ group }: { group: DestinationGroup }) {
  ...
  const completedCount = group.trips.filter(
  ...
  return (
    <Card className="border-border/50 overflow-hidden">
      <CardHeader className="py-4 px-4 pb-0">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {group.trips.length}{" "}
                {group.trips.length === 1 ? "visit" : "visits"}
          ...
          <div className="flex items-center gap-1 shrink-0">
            <Button
              ...
              onClick={() => setShowAerialView((v) => !v)}
            >
              <Video className="w-3.5 h-3.5 mr-1" />
              {showAerialView ? "Hide flyover" : "See from above"}
            </Button>
            <button
        ...
        {showAerialView && (
          <div className="px-0">
            <AerialViewPanel destination={group.destination} />
          </div>
        )}
      </CardHeader>
      {expanded && (
...
function JohnCat() {
  return (
    ...
    >
      ...
      <path
        d="M11.5,19.5 Q14.5,18 17.5,19.5"
        stroke="#c2410c"
        strokeWidth="1.5"
        fill="none"
        strokeLinecap="round"
      />
      <path
        d="M22.5,19.5 Q25.5,18 28.5,19.5"
        stroke="#c2410c"
        strokeWidth="1.5"
        fill="none"
        strokeLinecap="round"
      ...
      {/* Mouth */}
      <path
        d="M18.8,27.8 Q20,29.5 21.2,27.8"
        fill="none"
        stroke="#7c2d12"
        strokeWidth="0.8"
        strokeLinecap="round"
...
function AshleyCat() {
  return (
    ...
    >
      ...
      {/* Smile */}
      <path
        d="M18.8,27.6 Q20,29.8 21.2,27.6"
        fill="none"
        stroke="#7c2d12"
        strokeWidth="0.8"
        strokeLinecap="round"
...
function KarisCat() {
  return (
    ...
    >
      ...
      <path
        d="M14,14 L17,19 M26,14 L23,19 M17,19 L20,15 L23,19"
        stroke="#9a3412"
        strokeWidth="1.3"
        fill="none"
        strokeLinecap="round"
...
function AngelaCat() {
  return (
    ...
    >
      ...
      />
      <path
        d="M15.5,12.5 Q18.5,11 21.5,12.5"
        stroke="#c2410c"
        strokeWidth="1"
        fill="none"
        strokeLinecap="round"
      ...
      {/* Mouth */}
      <path
        d="M18.8,28.2 Q20,30.2 21.2,28.2"
        fill="none"
        stroke="#7c2d12"
        strokeWidth="0.8"
        strokeLinecap="round"
          ...
          <Card key={name} className="border-border/50">
            <CardContent className="py-4 px-4 flex flex-col items-center text-center gap-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {destinations > 0 &&
                  ` · ${destinations} ${destinations === 1 ? "place" : "places"}`}
              </p>
            </CardContent>
          </Card>
        );
  ...
  return (
    <div className="space-y-6">
      ...
      {!isLoading && groups.length > 0 && (
        <FamilyCountrySummary groups={groups} />
      )}
