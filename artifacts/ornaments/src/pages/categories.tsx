...
function CategoryColorEditor({ category }: { category: Category }) {
  ...
  const handleSave = async () => {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: getListOrnamentCategoriesQueryKey() });
  ...
  return (
    <div className="flex items-center gap-2 p-2 bg-muted/50 rounded-lg border border-border">
      ...
      <Badge
        ...
        style={{ backgroundColor: bgColor, color: textColor, border: `1px solid ${bgColor}` }}
      >
        {category.name}
      </Badge>
      <div className="flex gap-1 ml-auto">
        <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setIsEditing(false)}>Cancel</Button>
        <Button size="sm" className="h-7 text-xs" onClick={handleSave} disabled={updateColors.isPending}>
          {updateColors.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : "Save"}
        </Button>
      </div>
    </div>
  );
...
function CategoryActionMenu({ 
  ...
  const handleDelete = async () => {
    if (!confirm(`Are you sure you want to delete "${category.name}"? This will remove it from all items.`)) return;
    try {
    ...
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="sm" className="h-8 text-xs font-medium">Actions</Button>
        </DropdownMenuTrigger>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleRename} disabled={renameCat.isPending}>
              {renameCat.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Save
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={mergeOpen} onOpenChange={setMergeOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Move all items from "{category.name}" into another category, then delete "{category.name}".
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 flex flex-col gap-2 max-h-[300px] overflow-y-auto">
            {allCategories.filter(c => c.id !== category.id).map(c => (
              ...
              >
                <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${mergeTargetId === c.id ? 'border-primary' : 'border-input'}`}>
                  {mergeTargetId === c.id && <div className="w-2 h-2 rounded-full bg-primary" />}
                </div>
                <Badge
          ...
          <DialogFooter>
            ...
            <Button onClick={handleMerge} disabled={mergeCat.isPending || !mergeTargetId}>
              {mergeCat.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Merge
            </Button>
          </DialogFooter>
        </DialogContent>
...
export default function Categories() {
  ...
  const handleCleanup = async () => {
    if (!confirm("Delete all categories that aren't assigned to any items?")) return;
    try {
    ...
    <div className="mx-auto max-w-3xl space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Organize your collection with color-coded tags</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleCleanup} disabled={deleteUnused.isPending}>
            {deleteUnused.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            Cleanup unused
          </Button>
        ...
        <div className="p-4 rounded-xl border border-primary/30 bg-primary/5 mb-6">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-wrap items-end gap-4">
            <div className="flex-1 min-w-[200px]">
              ...
              <Input {...form.register("name")} autoFocus placeholder="e.g. Vintage, Star Wars..." />
            ...
            <div>
              ...
              <input type="color" {...form.register("bgColor")} className="h-10 w-16 p-1 border rounded cursor-pointer" />
            </div>
            <div>
              <Label className="mb-2 block">Text</Label>
              <input type="color" {...form.register("textColor")} className="h-10 w-16 p-1 border rounded cursor-pointer" />
            </div>
            <div className="flex gap-2">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 rounded-xl border border-dashed border-border bg-card">
          ...
          <p className="text-muted-foreground mt-1 max-w-sm mx-auto">Create categories to help filter and organize your ornaments.</p>
        ...
        <div className="bg-card rounded-xl border border-card-border shadow-sm divide-y divide-card-border overflow-hidden">
          {categories?.map((cat) => (
            <div key={cat.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 gap-4 hover:bg-muted/30 transition-colors">
