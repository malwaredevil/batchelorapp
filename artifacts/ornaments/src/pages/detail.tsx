...
import { 
  Loader2, ArrowLeft, Trash2, Camera, Lock, Unlock, 
  Search, Wand2, Download, Image as ImageIcon, X, Star
} from "lucide-react";
import { 
  useGetOrnament, 
  useUpdateOrnament, 
...
import type { OrnamentsOrnamentImage } from "@workspace/api-client-react";
...
export default function OrnamentDetail() {
  ...
  const { data: ornament, isLoading, isError } = useGetOrnament(id, { query: { enabled: !!id, queryKey: getGetOrnamentQueryKey(id) } });
  ...
  const addImage = useMutation({
    mutationFn: async (formData: FormData) => {
      const res = await fetch(`/api/ornaments/items/${id}/images`, {
        method: "POST",
        body: formData,
      });
      if (!res.ok) {
      ...
      return (await res.json()) as OrnamentsOrnamentImage;
    },
  });
  ...
  const lastSaved = useRef({ title: "", brand: "", series: "", year: "", notes: "", categories: "", lockedFields: "", aiDesc: "", dimensions: "", condition: "" });
  ...
  }, [title, brand, series, year, notes, categories, lockedFields, aiDesc, dimensions, condition, id, queryClient]);
  ...
  const LockIcon = ({ field }: { field: string }) => {
    ...
    return (
      <button 
        ...
        title={isLocked ? "Field locked from AI updates" : "Lock field to prevent AI overwrites"}
      >
  ...
  const handleDelete = async () => {
    if (!confirm("Are you sure you want to delete this ornament? This cannot be undone.")) return;
    setIsDeleting(true);
    try {
  ...
  if (isError || !ornament) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4">
        ...
        <Button variant="outline" onClick={() => setLocation("/")}>Return to collection</Button>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:block">
            <p className="text-sm text-muted-foreground font-medium uppercase tracking-wider">{brand} • {year}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button variant="destructive" size="sm" onClick={handleDelete} disabled={isDeleting} className="bg-destructive/10 text-destructive hover:bg-destructive hover:text-white border-transparent">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-8 items-start">
        ...
        <div className="space-y-4">
          <div className="relative aspect-square w-full rounded-2xl overflow-hidden bg-card border border-card-border shadow-md group">
            {ornament.imageUrl ? (
              <img src={ornament.imageUrl} alt={title} className="w-full h-full object-cover" />
            ) : (
            ...
            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
              <Button variant="secondary" className="rounded-full shadow-xl" asChild disabled={addImage.isPending}>
                <label className="cursor-pointer">
                  {addImage.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Camera className="mr-2 h-4 w-4" />} 
                  Add Photo
          ...
          {ornament.images && ornament.images.length > 1 && (
            <div className="flex gap-2 overflow-x-auto pb-2 snap-x">
              {ornament.images.map(img => (
                <div key={img.id} className="relative w-20 h-20 shrink-0 rounded-lg overflow-hidden border border-border group snap-start">
                  <img src={img.url} className={cn("w-full h-full object-cover", img.position === 0 && "opacity-50")} />
                  {img.position === 0 ? (
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1 backdrop-blur-sm">
                      <button onClick={() => handleSetPrimary(img.id)} className="p-1 text-white hover:text-primary transition-colors" title="Make primary">
                        <Star className="h-4 w-4" />
                      </button>
                      <button onClick={() => handleDeleteImage(img.id)} className="p-1 text-white hover:text-destructive transition-colors" title="Delete">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button variant="outline" className="h-auto py-3 flex flex-col gap-1 items-center justify-center bg-card shadow-sm" onClick={handleLookupPrice} disabled={lookupBookValue.isPending}>
              {lookupBookValue.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : <Search className="h-5 w-5 text-primary" />}
              <span className="text-xs">Price Check</span>
            </Button>
            <Button variant="outline" className="h-auto py-3 flex flex-col gap-1 items-center justify-center bg-card shadow-sm" onClick={handleReanalyze} disabled={reanalyze.isPending}>
              {reanalyze.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : <Wand2 className="h-5 w-5 text-primary" />}
          ...
          {ornament.bookValue != null && (
            <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 text-center">
              <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-1">Estimated Value</p>
              <p className="text-3xl font-serif font-bold text-primary">{formatCurrency(ornament.bookValue)}</p>
              <p className="text-[10px] text-muted-foreground mt-2">
                Source: {ornament.bookValueSource} <br/>
                Updated: {new Date(ornament.bookValueUpdatedAt!).toLocaleDateString()}
              </p>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {ornament.dominantColors?.map(c => (
              <Badge key={c} variant="outline" className="font-normal flex items-center gap-1">
                <span className="w-2 h-2 rounded-full inline-block" style={{backgroundColor: c}}></span> {c}
              </Badge>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-card border border-card-border p-6 rounded-2xl shadow-sm space-y-5">
            <div>
              <div className="flex items-center justify-between mb-1">
                <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center">Name <LockIcon field="name" /></Label>
              </div>
              <Input 
                value={title} 
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center mb-1">Brand <LockIcon field="brand" /></Label>
                <Input value={brand} onChange={e => setBrand(e.target.value)} className="bg-background" />
              </div>
              <div>
                <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center mb-1">Year <LockIcon field="year" /></Label>
                <Input value={year} onChange={e => setYear(e.target.value)} type="number" className="bg-background" />
              </div>
            </div>

            <div>
              <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center mb-1">Series / Collection <LockIcon field="seriesOrCollection" /></Label>
              <Input value={series} onChange={e => setSeries(e.target.value)} className="bg-background" />
            </div>
            
            <div className="grid grid-cols-2 gap-6">
              <div>
                <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center mb-1">Condition <LockIcon field="condition" /></Label>
                <Input value={condition} onChange={e => setCondition(e.target.value)} className="bg-background" placeholder="e.g. Mint in Box" />
              </div>
              <div>
                <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center mb-1">Dimensions <LockIcon field="dimensions" /></Label>
                <Input value={dimensions} onChange={e => setDimensions(e.target.value)} className="bg-background" placeholder="e.g. 4x3x2 in" />
              </div>
            </div>

            <div>
              <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center mb-2">Categories</Label>
          ...
          <div className="bg-card border border-card-border p-6 rounded-2xl shadow-sm space-y-5">
            <div>
              <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center mb-1">AI Description <LockIcon field="aiDescription" /></Label>
              <Textarea 
                value={aiDesc} 
            ...
            <div>
              <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center mb-1">Personal Notes <LockIcon field="notes" /></Label>
              <Textarea 
                value={notes} 
