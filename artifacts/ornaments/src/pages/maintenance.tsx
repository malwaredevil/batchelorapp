...
import { Loader2, Wrench, AlertCircle, RefreshCw, CheckCircle2, ChevronRight } from "lucide-react";
import {
...
export default function Maintenance() {
  ...
  const handleBulkReanalyze = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: getGetOrnamentStragglersQueryKey() });
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Keep your collection data rich and searchable</p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="mt-1.5">
                Automatically extract motifs, dominant colors, and generate descriptions for items missing them.
              </CardDescription>
            </div>
          ...
          {items.length > 0 && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                These items are missing AI descriptions, embedded features, or attributes that help them show up in searches and suggestions.
              </p>
      ...
      {items.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-xl font-serif font-bold text-foreground">Items needing attention</h2>
