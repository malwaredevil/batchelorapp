  ...
  const handleScannedCode = async (code: string) => {
    ...
    try {
      ...
      if (result.found) {
        ...
        sessionStorage.setItem("ornaments-add-prefill", JSON.stringify({
          name: result.name,
  ...
  return (
    <div className="mx-auto max-w-md space-y-6 pt-4">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-serif font-bold text-foreground">Scan Box</h1>
        <p className="text-muted-foreground mt-2">Scan the UPC barcode on the ornament box to autofill details</p>
      </div>
      ...
      {hasCamera ? (
        <div className="relative rounded-2xl overflow-hidden bg-black aspect-[3/4] sm:aspect-square shadow-xl shadow-black/10 border border-card-border">
          ...
          {!isScanning && !isLookingUp && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm">
              <Button onClick={startScanning} size="lg" className="rounded-full shadow-lg">
        ...
        <div className="bg-card rounded-2xl border border-card-border p-8 text-center shadow-sm">
          ...
          <p className="text-sm text-muted-foreground mt-2 mb-6">
            We couldn't access your device's camera. You can still enter the barcode manually below.
          </p>
        </div>
      )}
      ...
      <div className="pt-4 border-t border-border">
        <form onSubmit={handleManualSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              >
                {isLookingUp ? <Loader2 className="h-5 w-5 animate-spin" /> : <Search className="h-5 w-5" />}
              </Button>
            </div>
          </div>
        </form>
        ...
        <div className="mt-8 text-center">
          <Button variant="ghost" className="text-muted-foreground hover:text-foreground" onClick={() => setLocation("/add")}>
