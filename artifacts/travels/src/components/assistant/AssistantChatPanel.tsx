      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
        {messages.length === 0 && !isStreaming && (emptyState ?? (
          <div className="flex flex-col items-center gap-2 py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Hi, I'm <ElaineName />! Ask me anything about your trips, or whatever's on your screen.
            </p>
          </div>
        ))}
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-2.5 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            {msg.role === "assistant" && <ElaineAvatar size={avatarSize} className="mt-0.5" />}
            <div
        ...
        {isStreaming && (
          <div className="flex gap-2.5 justify-start">
            ...
            {streamingContent ? (
              <div className={`${bubbleWidthClass} whitespace-pre-wrap rounded-2xl rounded-tl-sm bg-muted px-3.5 py-2.5 text-sm leading-relaxed text-foreground`}>
                {streamingContent}
              </div>
            ) : statusMessage ? (
              <div className={`flex ${bubbleWidthClass} items-center gap-2 rounded-2xl rounded-tl-sm bg-muted px-3.5 py-2.5 text-sm text-muted-foreground`}>
        ...
        {pendingNavigate && (
          <div className="ml-8 flex flex-col gap-2 rounded-xl border border-primary/30 bg-primary/5 p-3">
            ...
            <div className="flex gap-2">
              <Button size="sm" className="h-7 text-xs" onClick={() => handleConfirmNavigate(onNavigated)}>
        ...
        {pendingActions.length > 0 && settings?.actionConfirmationMode === "all_at_once" && (
          <div className="ml-8 flex flex-col gap-2 rounded-xl border border-primary/30 bg-primary/5 p-3">
            <p className="text-xs text-muted-foreground">
              {pendingActions.length === 1 ? "1 thing" : `${pendingActions.length} things`} to confirm:
            </p>
            ...
            <div className="flex gap-2">
              <Button size="sm" className="h-7 text-xs" onClick={handleConfirmAll} disabled={confirmingAll}>
            ...
            {magnetResult && magnetResult.matches.length > 0 && (
              <div className="space-y-1.5">
                ...
                {magnetResult.matches.map((match) => (
                  <Link key={match.photoId} href={`/trips/${match.tripId}`} onClick={onNavigated}>
