      ...
      <CardContent className="space-y-3">
        ...
        {usedIn && usedIn.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-1">
            {usedIn.map((u) => (
              <a key={u.href} href={u.href} target="_blank" rel="noreferrer">
...
function GoogleApisDemoContent() {
  ...
  usePageAssistantContext(
    "google-apis-demo",
    `On the Google APIs demo page (owner-only) — a developer-facing showcase of every Google Maps Platform API enabled for this project (weather, air quality, pollen, nearby places, routes, static maps, street view, aerial view), demonstrated against ${demoTrip ? `the demo trip pinned at ${DEMO_DESTINATION}` : "sample coordinates in Paris (no demo trip created yet)"}. This is a dev/demo page, not a real trip-planning page.`,
  );

  const travelsTripHref = (id: number | string) =>
    `${base}modules/travels/trips/${id}`;
    ...
    <div className="mx-auto max-w-4xl space-y-6 p-4 md:p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <ApiCard
          ...
          usedIn={
            demoTrip
              ? [{ label: "View on trip", href: travelsTripHref(demoTrip.id) }]
              : []
          }
        >
        ...
        <ApiCard
          ...
          status="live"
          usedIn={[
            { label: "View on World Map", href: `${base}modules/travels/map` },
          ]}
        >
        ...
        <ApiCard
          ...
          status="live"
          usedIn={[
            { label: "World Map", href: `${base}modules/travels/map` },
            ...(demoTrip
              ? [
                  {
                    label: "Trip location map",
                    href: travelsTripHref(demoTrip.id),
                  },
                ]
              : []),
        ...
        <ApiCard
          ...
          status="live"
          usedIn={[{ label: "Login / Settings", href: `${base}account` }]}
        />
          ...
          usedIn={[
            {
              ...
              href: `${base}modules/travels/travel-calendar`,
            },
          ]}
        />
...
// Owner-only developer page — minimal chrome (just a back link), no header
// nav/app-switcher, since it's a diagnostic tool rather than a real feature.
