...
// ---------------------------------------------------------------------------
// Scaffolded collection sources
//
// Simple "collection" modules (pottery/quilting/ornaments-shaped CRUD
// resources) generated by scripts/src/scaffold-collection-module.ts are
// processed generically instead of getting a bespoke section above:
//   - every non-shared path is prefixed with `/<id>`
//   - every non-shared schema is prefixed with `schemaPrefix`
//   - operationIds that collide with any already-emitted operation are
//     renamed with the `<id>` word (same renameOpId rule as above)
// ---------------------------------------------------------------------------

interface CollectionSource {
  /** URL path prefix segment and collision-rename word (e.g. "gadgets"). */
  id: string;
  /** File name inside lib/api-spec/sources (e.g. "gadgets.yaml"). */
  file: string;
  /** PascalCase prefix applied to non-shared schema names (e.g. "Gadgets"). */
  schemaPrefix: string;
}
...
function main(): void {
  ...
  // Scaffolded collection sources — generic path + schema merge
  for (const src of COLLECTION_SOURCES) {
    const spec = loadSpec(src.file);
    const srcSchemas: Record<string, Json> = (spec.components?.schemas ??
      {}) as Json;

    const schemaRename = new Map<string, string>();
    for (const name of Object.keys(srcSchemas)) {
      if (!sharedSchemaNames.has(name)) {
        schemaRename.set(name, src.schemaPrefix + name);
      }
    }

    // Collision detection against every operationId already emitted
    const existingOps = new Set<string>();
    for (const item of Object.values(outPaths) as Json[]) {
      for (const [method, op] of Object.entries(item)) {
        if (!HTTP_METHODS.has(method)) continue;
        const opId = (op as Json)?.operationId;
        if (typeof opId === "string") existingOps.add(opId);
      }
    }
    const opRename = new Map<string, string>();
    for (const op of collectFeatureOpIds(spec.paths as Json)) {
      if (existingOps.has(op)) {
        opRename.set(op, renameOpId(op, src.id));
      }
    }

    for (const [p, item] of Object.entries(
      spec.paths as Record<string, Json>,
    )) {
      if (SHARED_PATHS.has(p)) continue;
      const newPath = `/${src.id}${p}`;
      const cloned = deepClone(item);
      rewriteSchemaRefs(cloned, schemaRename);
      applyOpIdRenames(cloned, opRename);
      if (outPaths[newPath] !== undefined) {
        throw new Error(
          `Duplicate path key after ${src.id} collection remap: ${newPath}`,
        );
      }
      outPaths[newPath] = cloned;
    }
    ...
    for (const [name, schema] of Object.entries(srcSchemas)) {
      if (sharedSchemaNames.has(name)) continue;
      const newName = schemaRename.get(name)!;
      const cloned = deepClone(schema);
      rewriteSchemaRefs(cloned, schemaRename);
      if (outSchemas[newName] !== undefined) {
        throw new Error(`Duplicate schema key: ${newName}`);
