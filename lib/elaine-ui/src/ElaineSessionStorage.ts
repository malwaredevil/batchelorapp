...
export function readElaineSessionHidden(): boolean {
  ...
  try {
    ...
    if (
      readWindowNameHidden() ||
      (preserveFallbackOnMissingStorageKey && volatileSessionHidden)
    ) {
      // A previous hidden write may have failed while reads still work
      // (read-only/quota-limited storage), or this may be a fresh module after
      // a full same-tab navigation. Do not let the missing key erase the
      // in-memory/window.name session fallback.
      volatileSessionHidden = true;
      preserveFallbackOnMissingStorageKey = true;
      writeWindowNameHidden(true);
      return true;
    }
