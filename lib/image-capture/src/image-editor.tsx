...
export function ImageEditor({ file, onSave, onCancel }: ImageEditorProps) {
  ...
  // ---------------------------------------------------------------------------
  // Load source image
  // We use an async function so every failure path is caught in one place.
  // The { imageOrientation:"from-image" } option applies EXIF rotation but is
  // not universally supported — it's tried first, with a plain call as fallback.
  // ---------------------------------------------------------------------------
  useEffect(() => {
    let cancelled = false;

    async function load() {
      // 1. Try createImageBitmap with EXIF-aware option (Chrome/Edge 84+)
      try {
      ...
      // 2. Try plain createImageBitmap (no EXIF correction)
      try {
        const bitmap = await createImageBitmap(file);
        if (!cancelled) {
          imgRef.current = bitmap;
        ...
        /* not available — fall through to <img> path */
      }

      // 3. Fallback: render via <img> → offscreen canvas → blob → bitmap
      try {
        ...
        await new Promise<void>((resolve) => {
          ...
          img.onload = () => {
            ...
            try {
              ...
              oc.width = img.naturalWidth || 1;
              oc.height = img.naturalHeight || 1;
        ...
        /* give up — image editor will remain in loading state */
      }
    }
