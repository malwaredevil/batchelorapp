...
/**
 * Shared Travels settings UI, used by both the Travels module (modules artifact)
 * and the unified hub Profile & Settings page (web artifact). Each app supplies
 * its own `usePageContext` hook (or omits it) so this lib stays free of any
 * app-local assistant-context wiring.
 */
type UsePageContext = (key: string, context: string | undefined) => void;
const noopPageContext: UsePageContext = () => {};
...
type LinkComponent = (props: LinkComponentProps) => ReactElement;
...
export function GmailSyncCard({
  ...
  usePageContext("settings-gmail", gmailContext);
