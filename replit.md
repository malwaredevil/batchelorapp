...
- Pre-publish checklist — run this automatically every time before creating a checkpoint (or immediately after), without waiting to be asked. Gated in stages; do not move to the next stage until the current one passes:
  ...
  **Stage 1 — review (manual steps first, then run the automated gate):**
  ...
  6. Run the automated pre-publish gate: `pnpm --filter @workspace/scripts run pre-publish`. It validates the actual local deploy candidate, including typecheck, lint, PII, workflow policy, and the composition-and-configuration boundary. A green check on an older GitHub `main` commit does not validate unsynced local changes. Do not skip this step or substitute individual manual checks. Fix every failure before proceeding to Stage 2.
  ...
  **Stage 3 — backup + GitHub sync (only after Stage 2 passes), in this exact order:**
  ...
  - 3b3. **Secrets boundary:** Replit Secrets is the runtime authority. Do not copy production application credentials to GitHub as a backup: GitHub Actions secret values cannot be read back, and the repository's validation workflows use disposable fixtures rather than production credentials. Add a GitHub secret only when a reviewed workflow explicitly requires it, and document that requirement in the PR.
  - 3c. Sync every changed repository file in one batch using `pnpm --filter @workspace/scripts run github-sync "commit message"`. The helper creates a `sync/YYYYMMDD-HHmm-<slug>` branch and one PR. Workflow files follow the same PR path as application code. Direct-to-`main`, mixed-batch splitting, per-file Contents API commits, and per-file push loops are prohibited. Excluded paths (.local/, .agents/, threat_model.md, .replit, .replitignore, replit.nix, .upm/) are enforced by the helper and must remain local.
  - 3d. Wait for every required check on the sync PR to finish successfully, including Guardrails, workflow policy, composition, tests, CodeQL, and database bootstrap when applicable. Merge only after they are green. Do not publish from the pre-merge branch.
  - 3e. In an isolated Replit background workspace, import the merged GitHub `main`. Review the import diff, then use Replit's apply/merge dialogue to update the Replit main workspace. The background task must not force-reset or directly rewrite the Replit main workspace and must preserve `.replit`, `.replit-artifact/`, `.local/`, `.agents/`, and other Replit-only files locally without pushing them to GitHub.
  - 3f. From Replit main, run `pnpm --filter @workspace/scripts run github-sync --verify-clean`. This read-only gate must report that every managed local file matches GitHub `main`; fix any mismatch through another PR.
  - 3g. Run `pnpm --filter @workspace/scripts run check-ci-status` and confirm the merged GitHub `main` commit is green. This is the final hard stop before publishing.
