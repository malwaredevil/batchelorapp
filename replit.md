...
- Pre-publish checklist — run this automatically every time before creating a checkpoint (or immediately after), without waiting to be asked. Gated in stages; do not move to the next stage until the current one passes:
  ...
  **Stage 3 — backup + GitHub sync (only after Stage 2 passes), in this exact order:**
  ...
  - 3c. Sync changed files to GitHub using `pnpm --filter @workspace/scripts run github-sync "commit message"`. The script runs prettier --write on every changed file and routes automatically based on file type — **the routing rule is hard-enforced in the script itself**:
    - **`.github/workflows/` files only → `--direct-to-main` flag required.** GitHub executes only the workflow version that lives on main; a branch copy is silently ignored. Use: `github-sync "msg" --direct-to-main`.
    - **All other files (no flag) → PR route.** Script creates a `sync/YYYYMMDD-HHmm-<slug>` branch and opens a PR. Merge only after all CI checks are green.
    - **Mixed batch (workflow + other files, no flag) → auto-split.** Script automatically: (1) pushes workflow files directly to main, then (2) opens a PR for all remaining files so CI runs against the freshly-updated workflow definitions. No flag needed — the split is detected and handled automatically.
    - **`--direct-to-main` with any non-workflow file → hard error.** Script aborts and lists the offending files. Non-workflow files must always go through a PR, no exceptions.
    - **Never** use the GitHub Contents API per-file (each file triggers its own CI run) or loop `git push` per file. Excluded paths (.local/, .agents/, threat_model.md, .replit, .replitignore, replit.nix, .upm/) are enforced by the script — Replit-specific files must never reach the public repo.
