...
- **GitHub vs Replit drift check (mandatory, every pre-publish, before any sync/import step):** directly compare the Replit workspace's current git HEAD against GitHub's `main` HEAD (commit ancestry via `GET /repos/malwaredevil/batchelorapp/compare/main...<replit-head-sha>`, plus a file-tree diff if ancestry alone is inconclusive). The expected, healthy state is "Replit is a clean fast-forward ahead of GitHub" (status `ahead`). If instead GitHub has commits/files Replit doesn't (`diverged` or GitHub `ahead`), or the comparison is ambiguous about which side is correct, **stop and ask the user which repo is the source of truth before taking any sync, push, or import action.** Never resolve this yourself by pulling, resetting, or rewriting Replit from GitHub, or by force-pushing Replit over GitHub — a past attempt to pull GitHub content into Replit deleted entire dependencies and the app is still recovering from the side effects. This check is in addition to, not a replacement for, the existing Apply/Merge-dialogue-only import path below.
