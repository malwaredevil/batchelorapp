...
- **GitHub vs Replit drift check (mandatory, every pre-publish, before any sync/import step):** because every GitHub merge is a squash merge (see Stage 3c), a commit-SHA/ancestry comparison (`GET /repos/malwaredevil/batchelorapp/compare/...`) will almost always show GitHub main at a SHA local git history never contains — that alone is expected, not drift. Do **not** treat a bare SHA/ancestry mismatch as evidence of a problem. The authoritative check is **content-level**: diff the actual file contents/blob SHAs of what the unfamiliar GitHub commit touched against the local working tree (`pull-from-github` or a per-file blob-SHA comparison). If content matches (mod trivial lockfile noise), local already has that commit's changes under different local SHAs — proceed normally, no import needed. **Only if real content differs** — GitHub has file content Replit's working tree doesn't — **stop and ask the user which repo is the source of truth before taking any sync, push, or import action.** Never resolve a genuine content difference yourself by pulling, resetting, or rewriting Replit from GitHub, or by force-pushing Replit over GitHub — a past attempt to pull GitHub content into Replit deleted entire dependencies and the app is still recovering from the side effects. This check is in addition to, not a replacement for, the existing Apply/Merge-dialogue-only import path below. See `.agents/memory/pre-publish-automation.md` ("Local main SHA vs GitHub main SHA will differ — that's expected") for the underlying reasoning.
...
- **Never let a cancelled/draft follow-up task quietly die.** Before proposing a new follow-up (or, if a task agent, before finishing your assigned work), search existing project tasks (including Archived/Cancelled and Draft/Proposed states) for one that already covers the same idea. If a match exists, re-surface that existing task (point the user at it) instead of silently skipping it or proposing a different alternative in its place. The user often cancels/leaves follow-ups in Draft only to unblock a push under time pressure — that is not the same as deciding it's unneeded. Keep re-surfacing a genuinely still-relevant task on future encounters until the user explicitly acts on it (accepts it, or says to drop it for good).
- **Collection item UI/UX should converge, not diverge, across collection types.** The user's stated preference: a collection item's page (add/detail/gallery/categories/etc.) should look and behave the same regardless of the collection (pottery, quilting, ornaments, and any future collection) so that adding a brand-new collection type is mostly configuration, not a UI rebuild. When building or fixing anything on a collection's item page, treat Pottery as the reference implementation, reuse `lib/collection-ui` (and other shared libs) instead of writing a bespoke per-app component, and prefer bringing a lagging app (e.g. Ornaments) up to the shared pattern over inventing a new one-off. Flag to the user when a fix would otherwise create yet another divergent pattern.
...
- Pre-publish checklist — run this automatically every time before creating a checkpoint (or immediately after), without waiting to be asked. Gated in stages; do not move to the next stage until the current one passes:
  ...
  **Stage 3 — backup + GitHub sync (only after Stage 2 passes), in this exact order:**
  ...
  - 3b4. **Pull GitHub-only changes back to local (required every session, before 3c):** Dependabot auto-merges (handled in 3b2) land directly on GitHub main without ever going through a Replit task/Apply-Merge step, so local can silently be behind. Run `pnpm --filter @workspace/scripts run pull-from-github` and review any files it writes — include them in the sync commit message in 3c. If it reports "No drift detected", proceed immediately. Skipping this risks `github-sync` in 3c silently reverting a Dependabot bump (see `.agents/memory/github-dependabot-sync-back-gap.md`).
  ...
  - 3c2. **Bot PR-suggestion check (required after every push to the sync PR, including follow-up commits):** run `pnpm --filter @workspace/scripts run apply-pr-suggestions`. It finds review comments from bot accounts (Copilot code review, Copilot Autofix for CodeQL, etc.) that include a suggestion block, applies the ones with a safe, non-overlapping, current position, and pushes them as a follow-up commit onto the _same_ `sync/…` branch — never a new branch/PR. Always relay its output to the user: which suggestions were applied (with the commit link), and which were left for manual "Commit suggestion" clicks (overlapping ranges or an outdated/unresolved position). After it pushes a commit, CI resets for that branch — go back to 3d.
  - 3d. Wait for GitHub CI to go fully green (all checks including CodeQL): `pnpm --filter @workspace/scripts run check-ci-status`. This is a hard stop — do not publish until this passes. **While waiting, re-run `apply-pr-suggestions` periodically (e.g. each time you re-poll CI status), not just once** — CodeQL Autofix comments in particular can land only after the CodeQL check itself finishes, i.e. partway through this wait. If a periodic re-check applies anything, CI resets again — keep polling until a pass produces no new commits AND check-ci-status is green.
  - 3e. **Security scan (required):** Compare against the session-start baseline counts. Any increase means a net-new finding was introduced this session — investigate before publishing.
    ...
    - Code scanning (CodeQL): `curl -s -H "Authorization: Bearer $GH_PAT" -H "Accept: application/vnd.github+json" "https://api.github.com/repos/malwaredevil/batchelorapp/code-scanning/alerts?state=open&per_page=50"` — fix real findings; dismiss false positives via `PATCH /repos/.../code-scanning/alerts/{n}` with `state: dismissed`, `dismissed_reason` (`false positive` / `used in tests` / `won't fix`), and a clear `dismissed_comment`. See `.agents/memory/github-security-and-branch-hygiene.md` for dismiss patterns specific to this repo.
      - **"Commit Suggestion" shortcut:** when GitHub shows a "Commit Suggestion" button next to a CodeQL Autofix or Copilot code-review finding in the PR UI, that's just a `\`\`\`suggestion`block on a review comment — step 3c2's`apply-pr-suggestions` already applies these automatically (bottom-to-top per file, skipping overlaps/outdated positions). Only point the button out to the user for the leftover cases it flags as manual (overlapping ranges or an outdated position).
