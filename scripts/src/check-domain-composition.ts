...
function buildCatalogToolNameSet(): Set<string> {
  ...
  // 4. Tools generated by scaffold:elaine-action live in standalone files under
  //    scaffolded-tools/ (each exports a ChatCompletionTool with an inline
  //    `name: "..."` literal) and are imported into the per-app action files /
  //    planner catalog. Scan the directory dynamically so a freshly scaffolded
  //    tool's POLICY_ROWS entry is recognized without editing this list.
  try {
    const scaffoldDir = "artifacts/api-server/src/elaine/scaffolded-tools";
    for (const file of readdirSync(resolve(root, scaffoldDir))) {
      if (!file.endsWith(".ts") || file.endsWith(".test.ts")) continue;
      const src = read(`${scaffoldDir}/${file}`);
      const reInline = new RegExp(INLINE_NAME_RE_SRC.source, "g");
      let m: RegExpExecArray | null;
      while ((m = reInline.exec(src)) !== null) {
        names.add(m[1]);
    ...
    // best-effort: directory only exists once a tool has been scaffolded
  }
  ...
  if (helperContents) {
    ...
    if (wrongValues !== null && wrongValues.size > 0) {
      const details = [...wrongValues.entries()]
        .map(
          ([constName, value]) =>
            `  - ${constName} (expected "${value.expected}", got "${value.got}")`,
        )
        .join("\n");
      violations.push(
