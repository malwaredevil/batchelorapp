...
export function isExcluded(filePath: string): boolean {
  ...
  // The dir entry itself (path ending in exactly ".../.replit-artifact") must
  // NOT be excluded here, or collectFiles never recurses into it to find
  // artifact.toml in the first place.
  const artifactDirIdx = segments.indexOf(REPLIT_ARTIFACT_DIR);
  if (
    artifactDirIdx !== -1 &&
    artifactDirIdx !== segments.length - 1 &&
