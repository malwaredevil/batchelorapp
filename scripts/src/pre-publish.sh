...
echo "  │ Stage 3c2 — Bot review loop (Copilot AND Codex are both required reviewers;  │"
echo "  │              during the DRAFT stage only Codex is awaited — Copilot does   │"
echo "  │              not auto-review drafts. Treat EVERY bot comment as a           │"
echo "  │              validation that must pass before merging. If additional bots   │"
echo "  │              are added in future, expand the expected-reviewer set below    │"
echo "  │              explicitly — do NOT rely on 'count >= 2'):                    │"
echo "  │                                                                             │"
echo "  │ !! HARD GATE — before ANY new commit or push to this branch, ALL of the   │"
echo "  │    following must be true simultaneously on the current HEAD:              │"
echo "  │      1. Every CI check-run is completed (none in_progress or queued)       │"
echo "  │      2. Required bots have posted a review with commit_id === HEAD SHA      │"
echo "  │           During the DRAFT stage: Codex only.                             │"
echo "  │           After promotion (Stage 3d.6): BOTH Copilot AND Codex.           │"
echo "  │           (GET /repos/.../pulls/:pr → head.sha; reject reviews tied       │"
echo "  │            to earlier commits regardless of when they were submitted)      │"
echo "  │      3. Every bot inline comment (excluding infrastructure bots:           │"
echo "  │           dependabot[bot], github-actions[bot],                            │"
echo "  │           github-advanced-security[bot], github-codeql[bot])               │"
echo "  │           has a non-bot reply                                              │"
echo "  │      4. Zero unresolved threads (confirmed via GraphQL)                   │"
echo "  │           Exception: within the Stage 3c2 loop, Steps B and C both       │"
echo "  │           push commits BEFORE their threads are closed. Requirements 3   │"
echo "  │           (all comments replied to) and 4 (zero unresolved threads) are  │"
echo "  │           merge gates for the NEXT gate cycle, not prerequisites for the  │"
echo "  │           Step B or C commits themselves. Push the fix first (Step C),   │"
echo "  │           or let apply-pr-suggestions push (Step B); then reply and       │"
echo "  │           resolve in Steps D-E before starting the next gate cycle.      │"
echo "  │    Spotted an obvious fix mid-wait? Note it — do NOT push early. Collect  │"
echo "  │    ALL CI failures + ALL bot comments first, then fix everything in one   │"
echo "  │    consolidated commit. Pushing early restarts CI and re-triggers bots    │"
echo "  │    on a commit they haven't finished reviewing, creating an infinite loop. │"
echo "  │                                                                             │"
echo "  │   Step A — wait for ALL bot reviews to finish (each is async):            │"
echo "  │              Poll GET /repos/.../pulls/:pr/reviews?per_page=100 and       │"
echo "  │              follow every Link-header next page, then filter to           │"
echo "  │              user.type=Bot, exclude infrastructure bots:                  │"
echo "  │                dependabot[bot], github-actions[bot],                       │"
echo "  │                github-advanced-security[bot], github-codeql[bot]           │"
echo "  │              Only count reviews whose commit_id matches the current PR      │"
echo "  │              head SHA (GET /repos/.../pulls/:pr → head.sha). Reviews      │"
echo "  │              tied to earlier commits must not satisfy this gate.           │"
echo "  │              Wait until EACH active reviewer bot has state=COMMENTED or    │"
echo "  │              APPROVED (not PENDING). Allow up to ~5 min per reviewer.     │"
echo "  │              Expected reviewer set (update this list if new bots are added): │"
echo "  │                copilot-pull-request-reviewer[bot]  (Copilot)              │"
echo "  │                chatgpt-codex-connector[bot]        (Codex)                │"
echo "  │              IMPORTANT — Copilot does NOT auto-review DRAFT PRs.          │"
echo "  │              During the draft stage (Stage 3c2), wait for Codex ONLY.     │"
echo "  │              After promotion to Ready-for-review (Stage 3d.5), Copilot    │"
echo "  │              triggers automatically; re-check for both bots in Stage 3d.6.│"
echo "  │              Codex does NOT auto-trigger at any stage — ALWAYS trigger:  │"
echo "  │                After opening the PR AND after every subsequent push,       │"
echo "  │                post a comment via:                                         │"
echo "  │                  POST /repos/.../issues/:pr/comments                       │"
echo "  │                  body: \"@codex please review this pull request\"           │"
echo "  │                Vary the phrasing slightly each time — exact-duplicate      │"
echo "  │                phrases are ignored by the bot. Examples:                   │"
echo "  │                  \"@codex review this PR\"                                  │"
echo "  │                  \"@codex can you review these changes\"                    │"
echo "  │                  \"@codex please take a look at this pull request\"         │"
echo "  │                Poll for chatgpt-codex-connector[bot] in reviews endpoint. │"
echo "  │              Do NOT proceed until the required bot(s) have posted          │"
echo "  │              (Codex only during draft; both after promotion).             │"
echo "  │                                                                             │"
echo "  │   Step B — apply inline suggestion blocks:                                 │"
echo "  │              pnpm --filter @workspace/scripts run apply-pr-suggestions      │"
echo "  │              Commits safe Copilot/Codex/CodeQL-Autofix diff-block          │"
echo "  │              suggestions. Run AFTER reviews arrive (Step A), not before — │"
echo "  │              suggestions are posted as part of the review, not before it.  │"
echo "  │              In Stage 3d.6 (post-promotion), pass --after <promotion ISO   │"
echo "  │              timestamp> so only post-promotion suggestions are applied;    │"
echo "  │              this prevents draft-stage [SKIP]ed suggestions on the same   │"
echo "  │              head SHA from being silently reapplied after promotion:       │"
echo "  │                pnpm ... apply-pr-suggestions --after <ready_for_review at> │"
echo "  │              If apply-pr-suggestions pushes a commit, do not restart yet: │"
echo "  │              route EVERY comment from that completed review (including    │"
echo "  │              auto-applied suggestions) through Steps C-E. Reply \"Applied │"
echo "  │              in commit <sha>\" to each auto-applied suggestion, give every│"
echo "  │              remaining comment a [FIX] or [SKIP] disposition, then       │"
echo "  │              resolve only after all comments are handled. THEN loop back │"
echo "  │              to Step A.                                                    │"
echo "  │                                                                             │"
echo "  │   Step C — address every inline comment (NO exceptions):                  │"
echo "  │              GET /repos/.../pulls/:pr/comments?per_page=100; follow every │"
echo "  │              Link-header next page before filtering or acting.             │"
echo "  │              Filter: user.type=Bot, excluding infrastructure bots above.   │"
...
echo "  │                POST /repos/.../pulls/:pr/comments/:id/replies              │"
echo "  │              (URL MUST include the PR number — omitting it returns 404.)   │"
echo "  │              with the decision + rationale (1-3 sentences).                │"
echo "  │              A pushed follow-up resets CI. Before looping back to Step A:  │"
echo "  │              reply to the comment that triggered the fix and resolve its  │"
echo "  │              thread via Steps D-E, THEN loop back to Step A              │"
echo "  │              (re-trigger Codex; wait for Codex only on the new HEAD       │"
echo "  │               during draft; both bots after promotion).                  │"
echo "  │                                                                             │"
echo "  │   Step D — after all comments are replied to, confirm none were missed:   │"
echo "  │              Re-fetch comments, verify every bot comment id (same           │"
echo "  │              infrastructure bot exclusion as Step C) has a reply          │"
echo "  │              from a non-bot author (i.e. the agent replied to each one).  │"
echo "  │                                                                             │"
echo "  │   Step E — resolve every review thread via GitHub GraphQL (REQUIRED       │"
echo "  │              before merging — GitHub branch protection blocks merge until  │"
echo "  │              all conversations are resolved). Never resolve a thread       │"
echo "  │              automatically: every thread, including excluded infrastructure │"
echo "  │              bot threads, needs an explicit [FIX] or [SKIP] disposition   │"
echo "  │              and a reply before it can be resolved:                        │"
...
echo "  │              3. Confirm 0 unresolved threads before proceeding.            │"
...
echo "  │              a stale/failing check). Promotion triggers the heavy CI suite.│"
echo "  │              Copilot re-reviews automatically; Codex does NOT — post an   │"
echo "  │              @codex comment manually immediately after promoting.           │"
echo "  │ Stage 3d.6 — consolidated merge gate (run BOTH in parallel, do not        │"
echo "  │              proceed to 3d2 until BOTH are satisfied):                    │"
echo "  │              ① CI — check-ci-status until fully green (all jobs including  │"
echo "  │                      E2E, CodeQL, elaine-capability-parity, etc.)          │"
echo "  │              ② Bots — promotion does NOT push a new commit, so the head    │"
echo "  │                      SHA is unchanged. Get the promotion timestamp first: │"
echo "  │                        GET /repos/.../issues/:pr/timeline                 │"
echo "  │                        find event=ready_for_review, use created_at        │"
echo "  │                      Only accept reviews with commit_id=head SHA AND      │"
echo "  │                      submitted_at >= promotion created_at. Then run       │"
echo "  │                      Steps B-E for any new post-promotion comments.       │"
echo "  │                      Re-trigger Codex manually after promoting            │"
echo "  │                      (post @codex comment — it never auto-triggers).      │"
echo "  │              Only move to 3d2 once CI is green AND 0 unresolved threads.  │"
