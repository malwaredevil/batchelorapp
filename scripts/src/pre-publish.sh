...
echo "  │ Stage 3c2 — Bot PR-suggestion check (after every push to the sync PR):     │"
echo "  │              pnpm --filter @workspace/scripts run apply-pr-suggestions      │"
echo "  │              Applies safe Copilot/CodeQL-Autofix suggestion blocks as a     │"
echo "  │              follow-up commit on the SAME branch; relay applied/manual      │"
echo "  │              suggestions to the user. A pushed commit resets CI — loop      │"
echo "  │              back into Stage 3d while still in Draft.                      │"
