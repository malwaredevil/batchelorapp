...
The travels app also includes an opt-in Gmail scanning feature: each household member may separately connect their own Gmail account (read-only, `gmail.readonly` scope, per-user OAuth tokens distinct from the Calendar OAuth grant) so the app can auto-detect flight/train/hotel confirmation emails, AI-extract structured trip data, and suggest linking them as trip documents. A companion manual mode lets a connected user browse/search their own inbox directly and choose any email to link. All Gmail access is strictly scoped to the connecting user's own tokens and own inbox — there is no cross-user Gmail access, even though trips themselves are household-shared.
...
- **Gmail OAuth tokens and inbox content** — per-user Gmail refresh/access tokens (`gmail.readonly` scope, separate grant from Calendar), plus any email subject/body/attachment content fetched during auto-scan or manual browsing. Compromise would expose a user's personal inbox contents and allow continued read access until the grant is revoked; leakage across users would violate the single-owner Gmail access guarantee even though trips are household-shared.
